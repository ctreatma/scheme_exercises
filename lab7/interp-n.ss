;; interp-n.ss
;;   Datatype, parser, environment definition and interpreter for Mini-Scheme-N
...
(define-datatype expression expression?
  ...
  (sequence-exp (exps (list-of expression?))))
    ...
    (cond
     ...
     ((pair? datum) 
	  ...
	  (if (eq? (car datum) 'let)
              ...
              (let-exp (get-ids (cadr datum)) 
                       (map parse (get-rands (cadr datum))) (parse (caddr datum)))
              (error 'parse "Bad let syntax: ~s" datum))
          (if (eq? (car datum) 'lambda)
              (if (eq? (length datum) 3)
                  (lambda-exp (cadr datum) (parse (caddr datum)))
                  (error 'parse "Bad lambda syntax: ~s" datum))
              (if (eq? (car datum) 'set!)
                  (if (eq? (length datum) 3)
                      (varassign-exp (cadr datum) (parse (caddr datum)))
                      (error 'parse "Bad set! syntax: ~s" datum))
                  (if (eq? (car datum) 'begin)
                      (sequence-exp (map parse (cdr datum)))
                      (if (eq? (car datum) 'letrec)
                          (make-letrec (get-ids (cadr datum))
                                       (get-rands (cadr datum))
                                       (caddr datum))
...
(define get-ids
  (lambda (lyst)
    (map (lambda (x) (car x)) lyst)))

(define get-rands
  (lambda (lyst)
    (map (lambda (x) (cadr x)) lyst)))

(define make-letrec
  (lambda (ids vals body)
    (let ([recids (map gensym ids)])
    (let-exp ids (map parse (map (lambda (x) 0) ids)) (let-exp recids (map parse vals)
                 (parse (append '(begin) (append (recbody ids recids) (list body)))))))))

(define recbody
  (lambda (ids vals)
    (cond ([null? ids] '())
          (else (cons (cons 'set! (cons (car ids) (list (car vals)))) (recbody (cdr ids) (cdr vals)))))))
   ...
   (lambda (exp env)
	...
	(if-exp (test-exp then-exp else-exp)
        ...
        (let-exp (ids rands body)
                 (eval-exp body (extend-env ids (map box (eval-rands rands env)) env)))
	...
	(app-exp (rator rands)
        ...
        (sequence-exp (exps)
                      (car (last-pair (map (lambda (x) (eval-exp x env)) exps))))
        (else (error 'eval-exp
...
(define init-env
  (extend-env '(nil True False) (map box '(() True False))
	      (extend-env
	       prim-op-names
	       (map box (map prim-proc prim-op-names))
	       the-empty-env)))
...
(define apply-proc
  (lambda (proc args)
	...
	(prim-proc (prim-op) (apply-prim-op prim-op args))
        (closure (formals body env)
                 (eval-exp body (extend-env formals (map box args) env)))
